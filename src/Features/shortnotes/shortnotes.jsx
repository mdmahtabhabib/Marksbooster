...

    {
      ...
      items: [
...

        {
          ...
          description:
            "Network of membranes involved in synthesis and transport of proteins, fats and some hormones.",
          table: {
...

        {
          ...
          description:
            "Double-membraned organelles involved in food synthesis and storage. They contain their own DNA and ribosomes.",
          table: {
            headers: ["Type", "Role"],
            rows: [
              [
                "Chloroplast",
                "Green, contains chlorophyll and performs photosynthesis."
              ],
              [
                "Chromoplast",
                "Contains yellow, orange or red pigments and colours flowers and fruits."
              ],
              [
                "Leucoplast",
                "Colourless plastids that store starch, oils and proteins."
              ]
            ]
          }
        },
            ...
            <thead>
              <tr>
                {item.table.headers.map((header, index) => (
                  <th
                    key={index}
                    className="border border-slate-200 bg-slate-50 px-[7px] py-[6px] text-left font-bold text-slate-800"
                  >
                    {header}
                  </th>
                ))}
              ...
              {item.table.rows.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  ...
                  {row.map((cell, cellIndex) => (
                    <td
                      key={cellIndex}
                      className="border border-slate-200 px-[7px] py-[6px] align-top whitespace-pre-line text-slate-700"
                    >
                      {cell}
                    </td>
                  ))}
  ...
  return (
    <section>
      ...
      <div
        className={
          section.type === "sub"
            ? "mb-[8px] mt-[16px] break-inside-avoid text-[13px] font-bold uppercase tracking-[0.06em] text-slate-500"
            : "mb-[10px] mt-[20px] break-inside-avoid border-l-[3px] border-[#0e5490] pl-[8px] text-[14px] font-bold uppercase tracking-[0.08em] text-[#0e5490]"
        }
      >
        {section.title}
      </div>
...
function ChapterNotes({ chapter }) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800">
      ...
      {/* HEADER — full-bleed bar; inner column keeps text on the 1080px grid */}
      ...
      <header className="sticky top-0 z-20 bg-[linear-gradient(165deg,#2379bb_0%,#125a9b_48%,#0a4a80_100%)] text-white shadow-[0_1px_2px_rgba(10,74,128,0.45),0_10px_28px_-12px_rgba(10,74,128,0.55)]">
        ...
        {/* soft top-left sheen so the bar reads as lit rather than flat */}
